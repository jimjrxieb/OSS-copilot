...
# Usage: ./scan-code.sh /path/to/repo
...
if command -v bandit &>/dev/null; then
    PYTHON_FILES=$(find "$TARGET" -name "*.py" -not -path "*/venv/*" -not -path "*/.venv/*" -not -path "*/node_modules/*" 2>/dev/null | head -1)
...
echo "  2. Scan for secrets: ./scan-secrets.sh $TARGET"
echo "  3. Scan dependencies: ./scan-dependencies.sh $TARGET"
